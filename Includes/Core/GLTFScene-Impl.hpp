...
namespace Core {
	...
	template <>
	inline void GLTFScene::GetValue<int>(const tinygltf::Value& value, const std::string& name, int& val)
	{
		if (value.Has(name))
		{
	...
	template <>
	inline void GLTFScene::GetValue<float>(const tinygltf::Value& value, const std::string& name, float& val)
	{
		if (value.Has(name))
		{
	...
	template <>
	inline void GLTFScene::GetValue<glm::vec2>(const tinygltf::Value& value, const std::string& name, glm::vec2& val)
	{
		if (value.Has(name))
		{
	...
	template <>
	inline void GLTFScene::GetValue<glm::vec3>(const tinygltf::Value& value, const std::string& name, glm::vec3& val)
	{
		if (value.Has(name))
		{
			auto vec = GetVector<float>(value.Get(name));;
	...
	template <>
	inline void GLTFScene::GetValue<glm::vec4>(const tinygltf::Value& value, const std::string& name, glm::vec4& val)
	{
		if (value.Has(name))
		{
			auto vec = GetVector<float>(value.Get(name));;
	...
	inline void GLTFScene::GetTextureID(const tinygltf::Value& value, const std::string& name, int& id)
	{
		if (value.Has(name))
		{
